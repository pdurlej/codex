...
impl ThreadContextPinsRequestProcessor {
    ...
    pub(crate) async fn thread_context_pins_list(
        &self,
        params: ThreadContextPinsListParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        self.thread_context_pins_list_inner(params)
            .await
            .map(|response| Some(response.into()))
    }

    pub(crate) async fn thread_context_pins_create(
        &self,
        params: ThreadContextPinsCreateParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        self.thread_context_pins_create_inner(params)
            .await
            .map(|response| Some(response.into()))
    }

    pub(crate) async fn thread_context_pins_update(
        &self,
        params: ThreadContextPinsUpdateParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        self.thread_context_pins_update_inner(params)
            .await
            .map(|response| Some(response.into()))
    }

    pub(crate) async fn thread_context_pins_delete(
        &self,
        params: ThreadContextPinsDeleteParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        self.thread_context_pins_delete_inner(params)
            .await
            .map(|response| Some(response.into()))
    }

    async fn thread_context_pins_list_inner(
        &self,
        params: ThreadContextPinsListParams,
    ) -> Result<ThreadContextPinsListResponse, JSONRPCErrorError> {
        ...
        Ok(ThreadContextPinsListResponse { data, next_cursor })
    }

    async fn thread_context_pins_create_inner(
        &self,
        params: ThreadContextPinsCreateParams,
    ) -> Result<ThreadContextPinsCreateResponse, JSONRPCErrorError> {
        ...
        Ok(ThreadContextPinsCreateResponse {
            pin: api_thread_context_pin_from_state(pin),
        })
    }

    async fn thread_context_pins_update_inner(
        &self,
        params: ThreadContextPinsUpdateParams,
    ) -> Result<ThreadContextPinsUpdateResponse, JSONRPCErrorError> {
        ...
        if !existing.iter().any(|pin| pin.pin_id == params.pin_id) {
            return Ok(ThreadContextPinsUpdateResponse { pin: None });
        }
        ...
        let pin = state_db
            ...
            .map(api_thread_context_pin_from_state);
        Ok(ThreadContextPinsUpdateResponse { pin })
    }

    async fn thread_context_pins_delete_inner(
        &self,
        params: ThreadContextPinsDeleteParams,
    ) -> Result<ThreadContextPinsDeleteResponse, JSONRPCErrorError> {
        validate_context_pin_id(&params.pin_id)?;
        let thread_id = parse_thread_id_for_context_pin_request(params.thread_id.as_str())?;
        let state_db = self.state_db_for_materialized_thread(thread_id).await?;
        ...
        Ok(ThreadContextPinsDeleteResponse { deleted })
    }
